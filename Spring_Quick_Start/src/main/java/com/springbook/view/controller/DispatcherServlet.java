...
public class DispatcherServlet extends HttpServlet {
	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		/* // Model2(MVC) 사용
		 * // 2. 클라이언트의 요청 path에 따라 적절히 분기처리 한다. if (path.equals("/login.do")) {
		 * 
		 ...
		 * 
		 * // 3. 화면 네비게이션 if (user != null) { System.out.println("로그인 성공");
		 ...
		 * session.invalidate();
		 * 
		 * // 2. 세션 종료 후, 메인 화면으로 이동한다. response.sendRedirect("login.jsp");
		 * 
		 * } else if (path.equals("/insertBoard.do")) {
		 * 
		 * System.out.println("글 등록 처리");
		 * 
		 * // 1. 사용자 입력 정보 추출 String title = request.getParameter("title"); String
		 * writer = request.getParameter("writer"); String content =
		 * request.getParameter("content");
		 * 
		 * // 2. DB 연동 BoardVO vo = new BoardVO(); vo.setTitle(title);
		 ...
		 * 
		 * // 2. DB 연동 BoardVO vo = new BoardVO(); vo.setSeq(Integer.parseInt(seq));
		 * 
		 ...
		 * 
		 * // 3. 응답 화면 구성 HttpSession session = request.getSession();
		 ...
		 * 
		 * // 1. 사용자 입력 정보 추출 // 2. DB 연동 처리 BoardVO vo = new BoardVO(); BoardDAO
